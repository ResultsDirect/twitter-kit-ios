...
{
    static NSString *sUUIDString = nil;
    static dispatch_once_t onceToken;

    dispatch_once(&onceToken, ^{
        NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
        sUUIDString = [userDefaults stringForKey:TWTRInstallationUUIDKey];
        if (!sUUIDString) {
            sUUIDString = [[NSUUID UUID] UUIDString];
            [userDefaults setObject:sUUIDString forKey:TWTRInstallationUUIDKey];
        }
    });
